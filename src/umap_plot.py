    ...
    with open(overlap_info, "w") as overlap_file:
        ...
        for category in unique_categories:
            ...
            for patient_id in adata_subset.obs[metadata_fields[0]].unique():
                ...
                overlap_file.write(f"{metadata_fields[0]} {patient_id} has {len(mutated_genes)} mutated genes.\n")
                gene_overlap[category].update(set(mutated_genes))
            overlap_file.write(f"{metadata_fields[0]} with the same {venn} of {category} have {len(gene_overlap[category])} overlap mutated genes.\n")
    return gene_overlap
